...
class Client {
    ...
    messageReceived(data) {
        ...
        var message
        var payload = str.substring(1, data.length - 1)

        // if command is a UPLOADBLOCK - we don't want to destroy the file block
        if (command === '17') {
            message = new MessageEvent(this, command, data.toString().substring(1))
        }
        else {
            message = new MessageEvent(this, command, data.toString().substring(1, data.length - 1))
        }
    ...
    handleMessage(message) {
        switch (message.command) {
            ...
            case MessageEvent.Type.RAW:
                ...
                break
            case MessageEvent.Type.UPLOADSTART:
                this.upload = new Upload(this, message)
                break
            case MessageEvent.Type.UPLOADBLOCK:
                this.upload.receive(this, message)
